...
import {
  ...
  SelectContent,
  SelectItem,
} from "@/components/ui/select"; // Added SelectPortal
...
export default function BookAppointment({ id }: { id: string }) {
  ...
  useEffect(() => {
    async function fetchData() {
      const response = await fetch(`/api/getdoctordetailbyid/${id}`);
  ...
  const onSubmit = (data: { timeSlot: string }) => {
    const slot = dummyDoctor.availableSlots
      .flatMap((s) => s.slots)
      .find((s) => `${s.start} - ${s.end}` === data.timeSlot);
    if (slot) {
      console.log(
        `Booking appointment with ${dummyDoctor.name} at ${slot.start} - ${slot.end}`
      );
  ...
  const selectedSlotDetails = selectedSlot
    ? dummyDoctor.availableSlots
        .flatMap((s) => s.slots)
        .find((s) => `${s.start} - ${s.end}` === selectedSlot)
    : null;

  const selectedDay = selectedSlotDetails
    ? dummyDoctor.availableSlots.find((s) =>
        s.slots.includes(selectedSlotDetails)
      )?.day
    : null;
      ...
      <div>
        <CardHeader className="p-6">
          ...
          <CardDescription className="text-gray-500 mt-2">
            Fill out the form below to schedule your appointment with Dr.{" "}
            {dummyDoctor.name}.
          </CardDescription>
        </CardHeader>
                      ...
                      {dummyDoctor.availableSlots.map((slot, index) => {
                        ...
                        return (
                          <div key={index} className="space-y-2 p-2">
                            ...
                            {index !==
                              dummyDoctor.availableSlots.length - 1 && (
